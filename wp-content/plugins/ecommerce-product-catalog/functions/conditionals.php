...
function is_ic_product_listing( $query = null ) {
	...
	if ( empty( $query ) ) {
		...
		if ( ( $query->is_post_type_archive( product_post_type_array() ) && ! $query->is_search() && ! is_ic_taxonomy_page( $query ) && ! is_ic_product_search( $query ) ) || is_home_archive( $query ) || is_custom_product_listing_page( $query ) ) {
			return true;
		}
	}
